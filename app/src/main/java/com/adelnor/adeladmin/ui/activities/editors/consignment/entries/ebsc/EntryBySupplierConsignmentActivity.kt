...
class EntryBySupplierConsignmentActivity : AppCompatActivity() {

    private var viewModel = EntryBySupplierConsignmentViewModel()
    ...
    private lateinit var documentData: SupplierConsignmentDocument
    private var tools = CommonTools()
    private lateinit var loadingDialog: AlertDialog

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_entry_by_supplier_consignment)
        ...
        etScanPurchaseOrder.visibility = GONE
        setupView()
    ...
    private fun setupView(){
        ...
        documentData = SupplierConsignmentDocument()
        ...
        etScanPurchaseOrder.setOnKeyListener{ _, keyCode, event ->
            if(keyCode == KeyEvent.KEYCODE_ENTER && event.action == KeyEvent.ACTION_UP){
                if(etScanPurchaseOrder.text.toString().isNotEmpty()){
                    loadingDialog.show()
                    processPurchaseOrderRead(etScanPurchaseOrder.text.toString().toInt())
                } else etScanPurchaseOrder.error = "Campo Requerido!"
            }
            false
        }
        ...
        if(result.isNotEmpty()){
            ...
            if(data.size>1){
                ...
                viewModel.getItemDetails().observe(this, Observer{ response ->
                    tools.hideKeyBoard(this)
                    if(response!=null){
                        val expirationDate = SimpleDateFormat("yyyy/MM/dd").parse(data[3].trim())
                        val expTerm = ((expirationDate.time - Date().time)/86400000)/30
    ...
    private fun processPurchaseOrderRead(result: Int){
        etScanPurchaseOrder.error = null
        viewModel.validateFolio(result)
        viewModel.getFolioValidation().observe(this, Observer { response ->
            loadingDialog.dismiss()
            if(response!=null){
                for (i in 0 until providersAdapter.count) {
                    if (providersAdapter.getItem(i)!!.supplierId == response.supplierId) {
    ...
    private fun confirmAddProduct(item: ItemExtended){
        ...
        builder.setPositiveButton(R.string.btn_accept) { dialog, _ ->
    ...
    private fun showDocumentDeleteConfirm(){
        ...
        builder.setNegativeButton(R.string.btn_accept) { dialog, _ ->
            loadingDialog.show()
            viewModel.deleteInServer(documentData.supplierIDQR, documentData.warehouseId, tools.getMacAddress(this))
