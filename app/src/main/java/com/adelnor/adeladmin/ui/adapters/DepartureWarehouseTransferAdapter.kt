...
import com.adelnor.adeladmin.model.db.DepartureWarehouseTransferItem
...
class DepartureWarehouseTransferAdapter(private var listener: ItemSelectedListener)
    : RecyclerView.Adapter<DepartureWarehouseTransferAdapter.CaptureViewHolder>() {

    var items: RealmResults<DepartureWarehouseTransferItem>? = null
        set(value) {
            field = value
    ...
    class CaptureViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView),
        ...
        fun bind(item: DepartureWarehouseTransferItem, listener: ItemSelectedListener) {
            tvQR.text = "${item.qrId}"
            tvAddress.text = "NA"
