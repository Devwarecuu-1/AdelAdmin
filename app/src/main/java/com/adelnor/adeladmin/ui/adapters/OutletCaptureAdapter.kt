...
class OutletCaptureAdapter(private var listener: ItemSelectedListener)
    : RecyclerView.Adapter<OutletCaptureAdapter.CaptureViewHolder>() {

    var items: RealmResults<OutItemExtended>? = null
        set(value) {
            field = value
    ...
    class CaptureViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView),
        ...
        fun bind(item: OutItemExtended, listener: ItemSelectedListener) {
            tvQR.text = "${item.qrId}"
